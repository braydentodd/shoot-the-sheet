...
# Foul semantics are NOT declared here: the ``core.pbp_events`` catalog
# (populated by ``discover-pbp`` and reviewed by hand) is the single
# authority mapping raw (EVENTMSGTYPE, EVENTMSGACTIONTYPE) keys to the
# canonical foul events (``o_standard_foul`` / ``d_standard_foul`` /
# ``elevated_foul``).  Keeping a source-side foul map would be a second
# source of truth that can drift from the catalog.
